						...
						foreach($row as $auser)
						{
							...
							?>
								...
								<!--	<h3 class="pmpro_member_directory_display-name">
									...
									<?php if(!empty($show_startdate)) { ?>
										<div class="column_wrap">
											<div class="label"><p><strong><?php _e('Join Date', 'pmpro'); ?>:</strong></p></div>
											<p class="pmpro_member_directory_date"><?php echo date(get_option("date_format"), $auser->membership_level->startdate); ?></p>
										</div>
									<?php } ?>
									<?php
									if(!empty($fields_array))
